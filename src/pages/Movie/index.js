      ...
      <Container className="w-60">
        ...
        <Row md={4}>
          {data.map((item, i) => {
            ...
            return (
              <Col className="d-flex align-items-end flex-column mt-3" style={{marginBottom:'30px'}} key={i}>
                <Row className="movie-item">
                  <Link className="movie-item-link" to={url}>
                  <Image className="movie-poster" src={item.poster} width={'100%'} height={440} />
                    {/* <div className='box detail-box'>Chi tiết</div> */}
                    <h3 className="fw-bold fs-movie-title mt-2" style={{paddingTop:'20px'}}>{item.title}</h3>
                  </Link>
                </Row>
                <Row className="mt-auto" style={{margin:'auto 0 0 0'}}>
                  <div className="px-0">
